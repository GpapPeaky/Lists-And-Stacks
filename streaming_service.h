...
/*__________________AUXILARY FUNCTIONS__________________*/

/*
 Extra functions to handle situations that cannot be handled locally in each .c file
the functions may have to be used first, at the top of the main function in the main.c
*/

/*
 * Intialises the sentinel pointer
 * for the users' list
 * 
 * Id is out of bounds ( -1 )
*/
...
/**
 * Print Function For Test Handling
*/
void newBlock(void);
...
/*__________________STACK METHODS__________________*/

/*
 All methods associated with stacks (push,top,pop ect...). They are included in the
			  _stack.c file. Used For The W event (Watch Movie Event).
*/

/**
 * Calculates The Size Of The WatchList Stack
*/
...
/*_______EXTERN VARIABLES_______*/

/*IT HAS COME TO MY ATTENTION THAT DECLARING VARIABLES AS EXTERN CAN LEAD TO OTHER PROBLEMS IN THE OTHER CODE
	     AND THEREFORE THEY ARE DECLARED HERE AND INTIALISED ELSEWHERE (iNSIDE THE OTHER .C FILES)*/

extern struct new_movie *newMoviesList;
extern struct new_movie *newMoviesTail;
extern struct new_movie *movieCategories[6]; 

extern struct new_movie* HORROR_TAIL;
extern struct new_movie* SCIFI_TAIL;
extern struct new_movie* DRAMA_TAIL;
extern struct new_movie* ROMANCE_TAIL;
extern struct new_movie* DOCUMENTARY_TAIL;
extern struct new_movie* COMEDY_TAIL;

extern struct user* userList; 
extern struct user userSentinel; 

/**
 * Array That Includes movieCategory_t Names For The Prints
*/
extern const char *categoryNames[6];

/*_______AUXILARY DEFINITIONS_______*/
